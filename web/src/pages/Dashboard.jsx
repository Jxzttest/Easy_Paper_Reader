...
function adaptPaper(p) {
  let status = 'pending';
  if (p.is_processed === 1 || p.is_processed === true) status = 'ready';
...
const Dashboard = () => {
  ...
  // 轮询解析中论文的 task 状态
  useEffect(() => {
    const processing = papers.filter(p => p.status === 'parsing' && p.taskId);
    ...
    return () => {
      const ids = new Set(processing.map(p => p.id));
      for (const [id, timer] of Object.entries(pollingRef.current)) {
        if (!ids.has(id)) { clearInterval(timer); delete pollingRef.current[id]; }
      }
    };
  }, [papers, fetchPapers]);

  useEffect(() => {
    return () => { for (const t of Object.values(pollingRef.current)) clearInterval(t); };
  }, []);
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 flex flex-col">
        <header className="bg-white/80 backdrop-blur-md border-b border-gray-200 sticky top-0 z-10">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
            ...
            <Button variant="outline" onClick={() => navigate('/knowledge-graph')} className="gap-2">
      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <input
              id="file-input" type="file" accept=".pdf" multiple className="hidden"
              onChange={(e) => {
                Array.from(e.target.files || []).forEach(file => handleUpload(file));
                e.target.value = '';
              }}
              ...
              >
                <div className="aspect-[4/3] bg-gray-100 relative overflow-hidden">
                  ...
                  <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                    {paper.status === 'ready' && (
                      <Button size="sm" variant="secondary" onClick={(e) => { e.stopPropagation(); handleOpenReader(paper.id); }}>
          ...
          <div className="flex items-center justify-center gap-2">
            ...
            <div className="flex items-center gap-1">
              {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                ...
                return (
                  <Button key={pageNum} variant={currentPage === pageNum ? 'default' : 'outline'} size="sm" className="w-8 h-8 p-0" onClick={() => handlePageChange(pageNum)}>
                    {pageNum}
                  </Button>
                );
              })}
